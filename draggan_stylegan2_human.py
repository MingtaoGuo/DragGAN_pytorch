...
from training.networks_stylegan2_human import Generator
import numpy as np 
...
def omega_p_r1(point, r1=3):
    ...
    for x in range(max(int(x0-r1), 0), min(int(x0+r1), 512)):
        ...
        for y in range(max(int(y_min), 0), min(int(y_max), 1024)):
...
def omega_p_r2(point, r2=12):
    ...
    for x in range(max(int(x0-r2), 0), min(int(x0+r2), 512)):
        for y in range(max(int(y0-r2), 0), min(int(y0+r2), 1024)):
            qs.append(np.array([x, y]))
    return qs
...
def motion_supervision(src_points, tar_points, F, M, F0, r1=3, lambd=20):
    F = functional.interpolate(F, [1024, 512], mode="bilinear")
    F0 = functional.interpolate(F0, [1024, 512], mode="bilinear")
    L_motion = 0
...
class DragGAN:
    def __init__(self, ckpt, device) -> None:
        self.G = Generator(z_dim=512, c_dim=0, w_dim=512, img_resolution=1024, img_channels=3, square=False).to(device)
    ...
    def gen_image(self, seed):
        ...
        img, feats = self.G.synthesis(latent, noise_mode="const")
...
if __name__ == "__main__":
    src_points = [np.array([232, 930]), np.array([330, 930])]
    tar_points = [np.array([186, 930]), np.array([369, 930])]
    # mask = points2mask(src_points, tar_points)
    # M = torch.tensor(mask[None, None], dtype=torch.float32).cuda()
    M = torch.ones([1, 1, 1024, 512]).cuda()
    seed = 600
